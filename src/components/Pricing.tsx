...
import { Layers, Rocket, Crown, CheckCircle2 } from "lucide-react";
import { Button } from "@/components/ui/button";
const plans = [{
  name: "Start",
  description: "Para quem quer começar ou organizar a operação nos marketplaces.",
  icon: Layers,
  features: ["Criação e otimização de anúncios", "Organização de catálogo e estoque", "Integração ERP + marketplaces", "Análise básica de preço, custo e imposto", "Relatório mensal de performance"],
  ...
  features: ["Tudo do Start", "Gestão ativa de performance", "Otimização contínua de anúncios", "IA analisando preços e margem", "Reuniões mensais", "Responsável pela conta"],
  ...
  icon: Crown,
  features: ["Tudo do Growth", "Estratégia de crescimento", "Gestão avançada de catálogo", "Foco em margem e escala", "Atendimento prioritário", "Projetos sob medida"],
  cta: "Falar com vendas",
  highlighted: false
}];
const Pricing = () => {
  return <section id="planos" className="py-24 px-6 relative pricing-bg">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-stretch max-w-6xl mx-auto mb-16">
          ...
          duration: 0.6,
          delay: index * 0.1
        }} className={`rounded-2xl p-8 border flex flex-col relative ${plan.highlighted ? "bg-neutral-900 border-neutral-800 shadow-2xl transform md:-translate-y-4 z-10" : "bg-card border-border hover:border-neutral-300 hover:shadow-2xl transition-all duration-300 group"}`}>
              {plan.badge && <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-brand-400 text-foreground text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full border border-brand-300 shadow-lg">
                  {plan.badge}
                </div>}

              <div className="mb-6">
                <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${plan.highlighted ? "bg-neutral-800" : "bg-brand-50"}`}>
              ...
              <Button variant={plan.highlighted ? "accent" : "outline"} className={`w-full ${plan.highlighted ? "font-bold py-3.5" : ""}`}>
                {plan.cta}
              </Button>
            </motion.div>)}
        </div>
